...
class M_Transaksi extends CI_Model {
	...
	public function cekstok(){
		...
		for($i=0;$i<count($this->cart->contents());$i++){
			...
			$stok = $this->db->where('kode_roti', $this->input->post('kode_roti')[$i])->get('roti')->row()->stok;
	...
	public function simpan_db(){
			...
			$this->db->where('kode_roti', $this->input->post('kode_roti')[$i])->update('roti', $u_stok);
			}
			...
			$s_dttransaski[] = array('kode_transaksi' => $ts->kode_transaksi,
									 'kode_roti'=>$this->input->post('kode_roti')[$i],
		...
		public function ambil_dts($kode_transaksi){
			...
			return $this->db->join('roti', 'roti.kode_roti = detail_transaksi.kode_roti')
